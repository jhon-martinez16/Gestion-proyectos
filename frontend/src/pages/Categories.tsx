...
export default function Categories() {
  ...
  const handleDesactivar = async (c: Category) => {
    ...
    try {
      await api.patch(`/categorias/${c.id}/desactivar`)
      loadData()
    } catch {
      setError("Error desactivando categoría.")
    }
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <button
          onClick={loadData}
          className="px-5 py-2 bg-[#16A34A] text-white rounded-xl font-medium hover:bg-[#15803D] transition"
        >
          Reintentar
        </button>
      </div>
  ...
  return (
    <div className="space-y-8 bg-[#F3FBF6] p-6 rounded-3xl">
      ...
      <div className="flex justify-between items-center bg-[#DDF7E6] px-6 py-4 rounded-2xl shadow-sm">
        <div>
          <h1 className="text-3xl font-bold text-[#14532D]">Categorías</h1>
